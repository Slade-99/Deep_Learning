...
log_file_name = "Federated Learning Training Log for LeViT\n"
...
#model_path = os.path.join("saved_models", "levit_round_2.pth")  
...
##### Load model is weights avilable ####
#model_path = os.path.join("saved_models", "levit_round_2.pth")  
#pretrained_levit.load_state_dict(torch.load(model_path))
...
sample_image_transformed = pretrained_levit_transforms(sample_image_pil)  # Apply transforms
output = pretrained_levit(sample_image_transformed.unsqueeze(0).to(device))
end_time = time.time()
latency = end_time - start_time
flops = FlopCountAnalysis(pretrained_levit, torch.randn(1, 1, 224, 224).to(device))
summary(pretrained_levit, input_size=(1, 1, 224, 224), col_names=["input_size", "output_size", "num_params", "trainable"], depth=3)
with open(log_file_path, 'a') as f:
    f.write("\nFinal Evaluation:\n")
