...
name = "Fed_Max_Custom_Architecture"
...
def train_local_model(local_model, global_model, dataloader, num_epochs, criterion, optimizer, lambda_max):
    local_model.train()
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in tqdm(dataloader, desc=f"Client Training Epoch {epoch + 1}"):
            ...
            per_sample_loss = criterion(outputs, labels, reduction='none')
            loss = per_sample_loss.mean()  # Average loss for backward pass
            
            # FedMax regularization: emphasize hard samples
            max_loss = torch.max(per_sample_loss)
            total_loss = loss + lambda_max * max_loss
...
for round_num in range(num_rounds):
    ...
    for client_id in selected_clients:
        ...
        local_weights = train_local_model(local_model, global_model,client_dataloaders[client_id], num_epochs, criterion, optimizer, lambda_max=0.1)
