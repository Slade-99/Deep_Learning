...
from embeddings import VisionRotaryEmbeddingFast
...
def vim_tiny(pretrained=False):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2()
    model = model.to(device)
    if pretrained:
        checkpoint = torch.load(str("/mnt/hdd/Research_works/Backbone_weights/Mamba_Backbone/ImageNet_Weights/vim_t_midclstok_ft_78p3acc.pth"))
        model.load_state_dict(checkpoint["model"])
    return model


def vim_small(pretrained=False):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = vim_small_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2()
    model = model.to(device)
    if pretrained:
        checkpoint = torch.load(str("/mnt/hdd/Research_works/Backbone_weights/Mamba_Backbone/ImageNet_Weights/vim_s_midclstok_ft_81p6acc.pth"))
        model.load_state_dict(checkpoint["model"])
    return model



def vim_base(pretrained=False):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = vim_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_middle_cls_token_div2()
    model = model.to(device)
    if pretrained:
        checkpoint = torch.load(str("/mnt/hdd/Research_works/Backbone_weights/Mamba_Backbone/ImageNet_Weights/vim_b_midclstok_81p9acc.pth"))
        model.load_state_dict(checkpoint["model"])
    return model
