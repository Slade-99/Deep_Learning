...
class Discriminator(nn.Module):
    def __init__(self, channels_img, features_d, num_class, img_size):
        ...
        self.embed = nn.Embedding(num_class, img_size*img_size)
...
class Generator(nn.Module):
    def __init__(self, channels_noise, channels_img, features_g, num_class, img_size, embed_size):
...
def test():
    ...
    disc = Discriminator(in_channels, 64, num_class=10, img_size=256)
    assert disc(x, torch.randint(0, 10, (N,))).shape == (N, 1, 1, 1), "Discriminator test failed"
    
    gen = Generator(noise_dim, in_channels, 64, num_class=10, img_size=256, embed_size=256*256)
